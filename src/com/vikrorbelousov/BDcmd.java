...
public class BDcmd {
    ...
    private void addMenu(LinkedList<String> listOper) {
        ...
        if (listOper.get(1).equals(Commands.MONEY)) {
            addMoney(listOper);
            return;
        }
    ...
    private void addMoney(LinkedList<String> listOper) {
        try {
            int money = Integer.parseInt(listOper.get(2));
            if (money < 0)
                throw new NumberFormatException();
            _user.addMoney(money);
            System.out.println("User id= " + _user.getParam().getId() + " add " + money);
            System.out.println("Balance=" + _user.getParam().getBalance());
        } catch (NullPointerException E) {
    ...
    private void addUser(LinkedList<String> listOper) {
    ...
    private void updateMenu(LinkedList<String> listOper) {
        ...
        if (listOper.get(1).equals(Commands.STATUS)) {
            updateStatus(listOper);
            return;
        }
        if (listOper.get(1).equals(Commands.SERVICE)) {
            updateSecvice(listOper);
            return;
        }

    }

    private void updateStatus(LinkedList<String> listOper) {
        try {
            int id = Integer.parseInt(listOper.get(2));
    ...
    private void updateSecvice(LinkedList<String> listOper) {
        try {
            _bd.outALL(BDWork.SERVICES_TABLE);
            Scanner scanner = new Scanner(System.in);
            System.out.println("Select ServiseId :");
            String inCmd = scanner.nextLine();
            int idService = Integer.parseInt(inCmd);
            if (_user.changeService(idService)) {
                System.out.println("Service update!");
                return;
            } else {
                System.out.println("Error update service!");
                return;
            }
        } catch (NullPointerException E) {
            System.out.println("User sing out!");
        } catch (ArrayIndexOutOfBoundsException E) {
            System.out.println("Error UPDATE SERVICE param");
        } catch (NumberFormatException E) {
            System.out.println("Error UPDATE SERVICE param");
        } catch (Exception E) {
            System.out.println("Eror UPDATE SERVICE param");
        }
    }
    ...
    private static String getHelp() {
        return "HELP - display this help\n" +
                ...
                "UPDATE SERVICE - open services list and update service";
    }
